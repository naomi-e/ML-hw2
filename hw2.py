...
def build_tree(data, impurity):
    ...
    while( not nodes_queue.empty() ):
        ...
        for attribute in range (num_attributes - 1): # -1 to exclude the lable..
...
def set_children(attribute, threshold, data, impurity):
    ...
    left_dn = DecisionNode(attribute, data[:,attribute].astype(float) <= threshold)
    right_dn = DecisionNode(attribute, data[:,attribute].astype(float) > threshold)
    
    print("set_children 2")
    left_data = data[np.where(data[:,attribute].astype(float) >= threshold)]
    right_data = data[np.where(data[:,attribute].astype(float) < threshold)]
...
#     print ((right_data))
    if ((left_data.shape[0] < 1) or (left_data.shape[1] < 1)):
        print("DATA IS LESS THAN 1 (1)")
        print("attribute: ", attribute, "threshold: ", threshold)
        left_impurity = 0
        return (None, None)
    else: 
    ...
    if ((left_impurity == 0) or (right_impurity == 0)):
        ...
        print("left_impurity:", left_impurity, "left_data.shape:", left_data.shape)
        print(left_data[:, -1])
        print("right_impurity:", right_impurity, "right_data.shape:", right_data.shape)
        print(right_data[:, -1])
        
        #print("right_impurity: right_data.shape:" %(right_impurity, right_data.shape))
        print(data.shape)
        print("attribute: ", attribute, "threshold: ", threshold)
